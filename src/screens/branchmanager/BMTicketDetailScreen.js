...
export default function BMTicketDetailScreen({ route, navigation }) {
  ...
  const confirmAssign = () => {
    ...
    update('ASSIGNED_FSE',
      ...
      { assignedFSE: selectedFSE, assignedFSEName: selectedFSEName, scheduledDate: dateStr }
    );
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        {ticket.status !== 'CLOSED' && (canApprove || canEscalate || canCloseRemote || canAssign || canHold) && (
          <AnimatedCard index={0} style={styles.actionsCard}>
            <Text style={styles.actionsTitle}>Available Actions</Text>
            {canApprove && <ActionBtn styles={styles} icon="checkmark-circle" label="Approve for Assessment" color={colors.success} onPress={approve} />}
            {canEscalate && <ActionBtn styles={styles} icon="trending-up" label="Escalate to Onsite" color={colors.info} onPress={escalate} />}
            {canCloseRemote && <ActionBtn styles={styles} icon="call" label="Close — Phone/Remote" color={colors.fse} onPress={closeRemote} />}
            {canAssign && <ActionBtn styles={styles} icon="person-add" label="Schedule & Assign FSE" color={colors.manager} onPress={() => setShowFSEModal(true)} />}
            {canHold && <ActionBtn styles={styles} icon="pause-circle" label="Place on Hold" color={colors.warning} outline onPress={() => setShowHoldModal(true)} />}
          </AnimatedCard>
        )}

        <AnimatedCard index={1}>
          <Card styles={styles} icon="person-outline" title="Client Information" accent={colors.manager}>
        ...
        <AnimatedCard index={2}>
          <Card styles={styles} icon="alert-circle-outline" title="Concern" accent={colors.manager}>
        ...
        {ticket.assignedFSEName && (
          <AnimatedCard index={3}>
            <Card styles={styles} icon="person-add-outline" title="Assignment" accent={colors.manager}>
        ...
        <AnimatedCard index={4}>
          <Card styles={styles} icon="time-outline" title="Activity Timeline" accent={colors.manager}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity style={[styles.confirmBtn, { backgroundColor: colors.manager }]} onPress={confirmAssign}>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity style={[styles.confirmBtn, { backgroundColor: colors.warning }]} onPress={confirmHold}>
...
function Card({ styles, icon, title, accent, children }) {
  return (
...
function ActionBtn({ styles, icon, label, color, outline, onPress }) {
  return (
    <PressableScale
      style={[
        styles.actionBtn,
...
const makeStyles = (colors, spacing, radius, shadow) => StyleSheet.create({
  ...
  actionsCard: { backgroundColor: colors.surface, borderRadius: radius.lg, padding: spacing.lg, marginBottom: spacing.md, borderWidth: 1, borderColor: colors.border, ...shadow.sm, gap: spacing.sm },
  ...
  modalCard: { backgroundColor: colors.surface, borderTopLeftRadius: radius.xxl, borderTopRightRadius: radius.xxl, padding: spacing.xl, paddingBottom: 40 },
  ...
  fseRow: { flexDirection: 'row', alignItems: 'center', gap: spacing.md, padding: spacing.md, borderRadius: radius.md, borderWidth: 1.5, borderColor: colors.border, marginBottom: spacing.sm },
  ...
  datePickerBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: colors.surfaceAlt, borderRadius: radius.md, borderWidth: 1.5, borderColor: colors.manager, padding: spacing.md, marginBottom: spacing.sm },
  ...
  datePickerChevron: { width: 32, height: 32, borderRadius: radius.sm, backgroundColor: colors.purpleBg, justifyContent: 'center', alignItems: 'center' },
  iosDoneBtn: { alignSelf: 'flex-end', paddingHorizontal: spacing.lg, paddingVertical: spacing.sm, marginBottom: spacing.sm },
  iosDoneBtnText: { fontSize: 16, fontWeight: '700', color: colors.manager },
  holdInput: { backgroundColor: colors.surfaceAlt, borderRadius: radius.md, borderWidth: 1, borderColor: colors.border, padding: spacing.md, fontSize: 14, color: colors.text, minHeight: 80, textAlignVertical: 'top', marginBottom: spacing.lg },
  ...
  confirmBtn: { flex: 2, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: spacing.sm, paddingVertical: 14, borderRadius: radius.md },
